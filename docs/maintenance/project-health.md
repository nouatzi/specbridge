...
- `npm run eslint10:readiness` runs in CI and reports ready/blocked state.
...
- CI uploads `health-summary` artifact for machine-readable check status, durations, and flaky retry counts.
...
- Review/open the monthly issue generated by `.github/workflows/health-snapshot.yml`.
- If automation is unavailable, open a maintenance issue from `.github/ISSUE_TEMPLATE/health_snapshot.md`.
